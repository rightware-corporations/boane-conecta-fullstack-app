...
public class QueueOperationsService {
    ...
    public QueueOperationResponse openDesk(UUID actorId, UUID queueId, UUID deskId) {
        User actor=user(actorId); MunicipalQueue queue=queue(queueId); QueueDesk desk=desk(queueId, deskId);
    ...
    public QueueOperationResponse closeDesk(UUID actorId, UUID queueId, UUID deskId) {
        User actor=user(actorId); queue(queueId); QueueDesk desk=desk(queueId, deskId); requireDeskActor(desk, actor);
    ...
    public QueueOperationResponse callNext(UUID actorId, UUID queueId, UUID deskId, String key) {
        validateKey(key); User actor=user(actorId); String fingerprint=hash(queueId+":"+deskId);
        ...
        MunicipalQueue queue=queue(queueId); QueueDesk desk=desk(queueId,deskId); requireDeskActor(desk,actor);
        if(queue.getStatus()!=QueueStatus.OPEN) throw new ResourceConflictException("QUEUE_NOT_OPEN");
    ...
    public QueueOperationResponse recall(UUID actorId, UUID ticketId) {
        User actor=user(actorId); QueueTicket ticket=ticket(ticketId);
    ...
    public QueueOperationResponse transfer(UUID actorId, UUID ticketId, UUID destinationQueueId, String reason) {
        ...
        User actor=user(actorId); QueueTicket source=ticket(ticketId);
        ...
        MunicipalQueue destination=queue(destinationQueueId);
