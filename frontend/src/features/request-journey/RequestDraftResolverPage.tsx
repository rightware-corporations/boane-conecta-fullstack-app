...
export default function RequestDraftResolverPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (detail.draft.status === 'IN_PROGRESS' || detail.draft.status === 'READY_FOR_REVIEW') {
        try { definition = await requestJourneyApi.pinnedDefinition(draftId); } catch { /* Owner-scoped pinned version unavailable: never render a different schema. */ }
      }
  ...
  const sameVersion = editable && definition?.serviceId === draft.serviceId && definition?.serviceVersionId === draft.serviceVersionId && definition?.formVersionId === draft.formVersionId;
  ...
  return <RequestJourneyShell title="Consultar rascunho" subtitle={sameVersion ? definition?.serviceTitle : undefined}>
    {loading ? <p role="status" aria-busy="true">A verificar o rascunho...</p> : error ? <div role="alert" className="border-l-4 border-destructive bg-surface p-4"><p>{error}</p>{isUuid(draftId) && <Button variant="outline" className="mt-4" onClick={() => setReloadKey(key => key + 1)}>Tentar novamente</Button>}</div>
      : draft && <section aria-labelledby="draft-heading" className="space-y-4">
        ...
        {draft.status === 'SUBMITTED' && draft.submittedRequestId ? <div role="status"><p>Este rascunho já foi submetido.</p><Button asChild className="mt-4"><Link to={`/municipe/pedidos/rascunhos/${draftId}/confirmacao`}>Ver confirmação</Link></Button></div>
          : !editable ? <p role="status">Este rascunho já não está disponível para edição. Os dados não serão alterados.</p>
            : !sameVersion ? <div role="alert" className="border-l-4 border-warning bg-surface p-4"><h3 className="font-semibold">Continuação indisponível</h3><p className="mt-2 text-sm leading-6">Não foi possível confirmar a definição fixada deste rascunho. As respostas antigas não serão apresentadas num formulário diferente. Nenhum dado foi alterado.</p><Button variant="outline" className="mt-4" onClick={() => setReloadKey(key => key + 1)}>Verificar novamente</Button></div>
              : <div role="status" className="space-y-3 border-l-4 border-primary bg-surface p-4"><h3 className="font-semibold">Rascunho disponível</h3><p className="text-sm leading-6">A definição fixada neste rascunho está disponível. A submissão requer validação e aceitação da declaração na revisão.</p><div className="flex flex-wrap gap-3"><Button asChild><Link to={`/municipe/pedidos/rascunhos/${draftId}/elegibilidade`}>Verificar elegibilidade</Link></Button><Button asChild variant="outline"><Link to={`/municipe/pedidos/rascunhos/${draftId}/formulario`}>Formulário</Link></Button><Button asChild variant="outline"><Link to={`/municipe/pedidos/rascunhos/${draftId}/documentos`}>Documentos</Link></Button><Button asChild variant="outline"><Link to={`/municipe/pedidos/rascunhos/${draftId}/revisao`}>Revisão</Link></Button></div></div>}
