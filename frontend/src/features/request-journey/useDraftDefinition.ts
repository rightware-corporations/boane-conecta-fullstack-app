...
export function useDraftDefinition(draftId: string) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const definition = await requestJourneyApi.definition(detail.draft.serviceId);
      if (definition.serviceVersionId !== detail.draft.serviceVersionId || definition.formVersionId !== detail.draft.formVersionId)
