...
export type QueueConfig = { id: string; name: string; locationCode: string; departmentId: string; serviceId: string | null; mode: 'APPOINTMENT_REQUIRED' | 'WALK_IN_ALLOWED' | 'HYBRID'; status: 'OPEN' | 'PAUSED' | 'CLOSED'; version: number; desks: Array<{ id: string; code: string; displayName: string; status: string; currentStaffUserId: string | null; version: number }>; staffUserIds: string[] };
...
export type QueueStaffOption = { id: string; fullName: string; email: string; roles: string[] };
...
export const listQueueStaffOptions = async () => value(await api.get<ApiResponse<QueueStaffOption[]>>('/admin/queues/staff-options'));
...
export const assignQueueStaff = async (queueId: string, userId: string) => value(await api.post<ApiResponse<QueueConfig>>(`/admin/queues/${queueId}/staff-scopes`, { userId }));
export const revokeQueueStaff = async (queueId: string, userId: string) => value(await api.delete<ApiResponse<QueueConfig>>(`/admin/queues/${queueId}/staff-scopes/${userId}`));
