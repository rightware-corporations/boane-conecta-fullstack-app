...
| `ADDRESS` | JSON object ou null | **BLOCKED para edição do próprio campo.** O backend só verifica `isObject()`; não existe contrato de propriedades, labels nem obrigatoriedade de subcampos. Os campos vizinhos da mesma etapa continuam editáveis e a resposta de endereço existente é preservada; um endereço obrigatório em falta impede concluir o pedido. Requer componentes de endereço publicados e validados no backend. |
